<template>
    <div id="login" class="flex py-20 border-b-4 items-center">
      ...
      <div class="form-container w-1/2 px-10 py-20 bg-gray-100 rounded">
        ...
        <p class="text-lg">
          <span
            ...
            class="text-red-600 font-bold cursor-pointer hover:text-black"
          >
            Quên mật khẩu ?
          </span>
          <span v-else @click="onResume" class="text-red-600 font-bold cursor-pointer hover:text-black">
            Tiếp tục quá lấy lại mật khẩu({{ onStep }}/3)
          </span>
        </p>
        <p class="text-lg">
          Bạn chưa có tài khoản?
          <span @click="$router.push('/register')" class="text-red-600 font-bold cursor-pointer hover:text-black">
            Đăng kí ngay !
          </span>
        </p>
      </div>
      <vs-popup :active.sync="isForgetPassword" title="Quên mật khẩu" class="text-center">
        ...
        <input
          class="w-1/2 text-center border-2 rounded p-2 text-lg mt-10"
          placeholder="Số điện thoại"
          v-model="phoneForget"
        />
        <div class="mt-6">
      ...
      <vs-popup title="Nhập mã code" class="text-center" button-close-hidden :active.sync="isEnterCode">
        ...
        <input
          :maxlength="6"
          type="text"
          class="w-1/2 h-16 tracking-widest text-2xl bg-gray-200 text-center"
          v-model="code"
        />
      ...
      <vs-popup title="Mật khẩu mới" button-close-hidden :active.sync="isConfirmNewPassword">
        ...
        <vs-input
          ...
          type="password"
          class="w-full tracking-widest bg-gray-200 mb-8"
          v-model="newPassword"
        />
        <vs-input
          label-placeholder="Nhập lại mật khẩu"
          type="password"
          class="w-full tracking-widest bg-gray-200"
          v-model="rePassword"
        />
        <div class="mb-2 mt-10">
          <vs-button :disabled="!isValidNewPassword" color="danger" class="px-10" @click="onConfirmNewPassword">
            Xác nhận mật khẩu
          </vs-button>
        </div>
      </vs-popup>
  ...
  export default {
    ...
    methods: {
      ...
      async onConfirmForgetPassword() {
        const res = await this.confirmForgetPassword({
          phone: convertPhone(this.phoneForget),
      ...
      async onConfirmNewPassword() {
        const res = await this.confirmNewPassword({
          phone: convertPhone(this.phoneForget),
